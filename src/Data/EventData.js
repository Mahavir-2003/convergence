const Events = [
    {
        EventId: 1,
        EventName: 'CIVESTA',
        MajorEventLogo: 'civesta.png',
        subEvents: [
            {
                subEventId: 1,
                subEventName: 'CivilInfra@2047',
                eventLogo: 'math/math.png',
                eventDescription: 'The objective behind organisation of poster making competition is to bring out the creative expression of students and to develop a vision in the field of Infrastructure in India.',
                eventRules: [
                    "Each group (maximum 2 participants) can post only one Poster that should be printed/drawn from the home.",
                    `The poster must be made on the Chart Paper or 42" X 27" inch  sheet with margins (0.5 inches).`,
                    "Poster can be made using pencil, crayon, watercolour, oil paint, etc. Photographs, wires, other 3D objects, and print-outs are also acceptable. ",
                    "The Topic of the poster and theme should be clearly mentioned at the Top Margin of the Poster.",
                    "Name, Class, College of the Participants must write on the bottom-left portion of the poster.",
                    "These details should be the same as in the registration form.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Patel Dhruti',
                        contact: '9106107115'
                    },
                    {
                        name: 'Patel Birju',
                        contact: '9427815290'
                    },
                ],
                facultyContact: [
                    {
                        name: 'John Doe',
                        contact: '1234567890'
                    },
                ],
                pricePool: {
                    first: '3000',
                    second: '2000',
                    third: '1000'
                },
            },
            {
                subEventId: 2,
                subEventName: 'Float the Boat',
                eventLogo: 'sub/float_the_boat.png',
                eventDescription: 'Participants have to build a Boat by using Paper, Wooden Ice cream stick and Fevicol Only. Material for the same will be provided by us on same day of competition. Maximum dimension of the Boat: Length = 25 cm; Width = 15 cm, height = 15 cm. Load will be applied laterally at the center of the boat. Model which carries maximum load without sinking will be declared as winner.',
                eventRules: [
                    "Only boat has to be built.",
                    "Materials will be provided from the institute on same day of competition. Team can physically alter by cutting, notching, sanding or bending to make proper fitting connection.",
                    "Materials may not be painted, covered, coated, or soaked in anything except water 3 to 5 members are allowed in a team.",
                    "Only First 25 teams are allowed to participate on First come first get basis.",
                    "Maximum dimension of the Boat: Length = 25 cm; Width = 15 cm, height = 15 cm.",
                    "Violation of any rules will be followed by instant disqualification Teams must submit their Boats sharply on or before 12:00 noon for inspection.",
                    "15 minutes extra shall be provided to modify the boat if it does not meets the given specifications.",
                    "Participants must carry their own scissors, scale and other craft related items.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Parshwi D Shah',
                        contact: '7999498491'
                    },
                    {
                        name: 'Kunj Patel',
                        contact: '9558990069'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Jagdish Khatik',
                        contact: '8741035420'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 3,
                subEventName: 'Knowledge Knockout',
                eventLogo: 'math/1536.png',
                eventDescription: '"Knowledge Knockout is a technical quiz competition of Civil Engineering knowledge discipline and GK concerned with Civil Engineering Knowledge. Round 1: Correct me Correct me round will display images and will ask for the correct answer Round 2: Identify me Identify me round will display images and will ask for the correct identification."',
                eventRules: [
                    "In each group, maximum 3 participants may be enrolled.",
                    "Round 1 and Round 2 will have questions displayed on the projector screen. Participants need to answer them in the given time.",
                    "The score and time limit will be shared before the start of the event",
                ],
                cordinatorsContact: [
                    {
                        name: 'Khushi Shah',
                        contact: '6352003794'
                    },
                    {
                        name: 'Patni Bhavesh',
                        contact: '7041512647'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Kamlesh Damdoo',
                        contact: '8460270839'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 4,
                subEventName: 'Cantilever Crane',
                eventLogo: 'math/math.png',
                eventDescription: '"1. Students had to participate in group of five.2. This event consist of preparing a  cantilever Crane structure with the provided number of wooden sticks with the accessories provided. Total Number of sticks provided: 100 max.3. The structure will be tested as per load carried by the structure and various other criteria. 4. The heavier the load carrying capacity, the winner will be decided accordingly."',
                eventRules: [
                    "Students count should be 5.*T& C Apply",
                    "Students should be present on 10 min before their event starts.",
                    "A Cantilever crane structure has to be prepared by the team with the help of wooden sticks and accessories provided.",
                    "Criteria for structure: 1. Height of the structure:90 to 120 cm for height and 50cm to 90cm for cantilever section. 2. Cross section of structure in plan for column: 15cm*15cm column and 10cm*10cm for cantilever portion of the crane. 3. Weight Criteria 4. Preparation Time Criteria 5. No. of Sticks Utilization criteria",
                    "Any team caught in misbehavior with the coordinators are eligible to get disqualified.",
                    "Further Criteria will be modified if as needed."
                ],
                cordinatorsContact: [
                    {
                        name: 'Rishabh Jaiswal',
                        contact: '7860476333'
                    },
                    {
                        name: 'Parmar Navin',
                        contact: '9925222634'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Hitesh A.Sonarghare',
                        contact: '9737704296'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },

        ]
    },
    {
        EventId: 2,
        EventName: 'HUMANICA',
        MajorEventLogo: 'humanica.jpg',
        subEvents: [
            {
                subEventId: 1,
                subEventName: 'Word Wizard Wars',
                eventLogo: 'math/math.png',
                eventDescription: "There will be four levels of competitions named word wizard wars. The topic of event is basic and general English language.",
                eventRules: [
                    "1. Participants must not use Mobile phones or any other digital devices during the competition. ",
                    "2. If anyone is found to have copied answers from Google, they will be disqualified. ",
                    "3. Id card is mandatory for all participants. ",
                    "4. Spot Entries are not allowed.",
                    "5. All participants must treat each other and the organisers with respect. ",
                    "6. Judges' decisions are final.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Tirth Patel',
                        contact: '9409638544'
                    },
                    {
                        name: 'Suhada Khan',
                        contact: '9101236181'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Abhishek Raval',
                        contact: '7016018639'
                    },
                ],
                pricePool: {
                    first: '3000',
                    second: '2000',
                    third: '1000'
                },
            },
            {
                subEventId: 2,
                subEventName: 'Talk on Mathematics',
                eventLogo: 'math/math.png',
                eventDescription: " The game is in a single round. Each team or each participant will get only one chance to present. Student must chose the topics from (1) History of Mathematics, (2) Conceptual topic of Mathematics, (3) Application of Mathematics in Engineering, (4) Contribution of Mathematician in Society. The results will be declared on the same day.",
                eventRules: [
                    "1. Only first 20 students or team are allowed for this events.",
                    "2. Each team or each participant will get 7 minutes for presentation.",
                    "3. Final decision regarding any matter lies in the hands of the judges and is binding.",
                    "4. Any students violating the above rules is subject to direct elimination."
                ],
                cordinatorsContact: [
                    {
                        name: 'Panchal Om Tarangbhai ',
                        contact: '6358835535'
                    },
                    {
                        name: 'Kansagara Krish Kalpeshbhai',
                        contact: '8390690850'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Dr. Rakesh R. Darji',
                        contact: '9879750932'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },


            {
                subEventId: 3,
                subEventName: 'MathQuizzerie ',
                eventLogo: 'math/math.png',
                eventDescription: "The game is in two rounds: elimination and final. Stage I: Elimination round There will be a written test for all participating students.The written test will be of 20 minutes.Questions will check the mathematical ability of participating students.Top 10 students will be selected for final round based on points they have gained in written test. Stage II: Final roundIn Final round there are 3 sub rounds listed below.or Each question student have only 30 seconds in each round. For correct answer,the students will earn 10 points. In case of wrong answer, the team will lose 5 points.Students can pass the question. In case of passing, if any students answer the questioncorrectly will eligible to earn 5 points. In case of wrong answer, the students will lose 5points.There will be 2 rounds of asking questions to top 10 students.After these two rounds of asking questions, 5 students from top 10 will be eliminated. Rest of top 5 students will have to play buzzer round. For Each question student haveonly 30 secs. Winner will be decided at the same time when all the rounds get completed.",
                eventRules: [
                    "1. It is an individual event.",
                    "2. Only first 150 students is allowed for this events",
                    "3. Final decision regarding any matter lies in the hands of the judges and is binding.",
                    "4. Any students violating the above rules is subject to direct elimination.",
                    "5. Elimination round is applicable if number of participating teams are more than 10."
                ],
                cordinatorsContact: [
                    {
                        name: 'Parmar Madhvi Jaydeepsinh ',
                        contact: '7016501740'
                    },
                    {
                        name: 'Patel Krisha GirishKumar',
                        contact: '7043451171'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Dr. Rakesh R. Darji',
                        contact: '9879750932'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },

        ]
    },
    {
        EventId: 3,
        EventName: 'INFOCRATS',
        MajorEventLogo: 'infocrafts.png',
        subEvents: [
            {
                subEventId: 1,
                subEventName: 'POSTER PRESENTATION',
                eventLogo: 'math/math.png',
                eventDescription: 'Participants are free to choose and select any technical topic of their interest. Give first priority to the latest technologies. The size of the posters should be A1 sized (841*594 mm) sheet.',
                eventRules: [
                    "Poster Size must be A1 size (841*594 mm) (33.1 x 23.4 in). You can use Paper or chart papers or any other materials as per requirements. You can print-out pictures but content should be written manually.",
                    "Poster should be written in English and well Formatted.",
                    "Maximum 2 Participants in each Team. (One Participant can also take part)",
                    "Both participants must be registered for event.",
                    "Each team will have to make only one poster.",
                    "Poster should not be copied as it is from external sources.",
                    "Topic to be present should be technical and related to engineering studies.",
                    "Text font should be easy to read. (Calibri, Arial, Times New Roman etc.)",
                    "Name of participant(s) should be written right bottom corner in Times New Roman font.",
                    "Any commercial/financial help (for poster making) will not provide by college.",
                    "Content is important, but keep is concise.",
                    "Poster evaluation is based on judge’ opinion"
                ],
                cordinatorsContact: [
                    {
                        name: 'Dwija patel',
                        contact: '8200761750'
                    },
                    {
                        name: 'Zeel patel',
                        contact: '9265956990'
                    },
                ],
                facultyContact: [
                    {
                        name: 'PROF. RITESH UPADHYAY',
                        contact: '9685003861'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 2,
                subEventName: 'PAPER PRESENTATION',
                eventLogo: 'math/math.png',
                eventDescription: 'The paper presentation aims at giving a platform to young engineers to fuel their brains to ideate ideas that never existed. The event intends to encourage inventions, innovation, and research to promote their application for development in the computer and IT sectors of Engineering. Participants of this event are expected to prepare a paper about a real-life project topic or area and present it in front of experts.',
                eventRules: [
                    "Each team consists of at most 2 members only.",
                    "Duration for the paper presentation is a maximum of 15 minutes.",
                    "Use only Microsoft PowerPoint presentation",
                    "The paper should be in IEEE format.",
                    "Hard copies of the paper are to be submitted before presentation to the expert.",
                    "Any student from any college and from any branch is allowed to participate.",
                    "Theme of the paper: only computer and IT technical domains allow.",
                    "Spot Entries are not accepted."
                ],
                cordinatorsContact: [
                    {
                        name: 'Aparnathi Chintan',
                        contact: '8128952544'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Hiteshri Modi & Zarana Patel',
                        contact: '8460196083'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 31,
                subEventName: 'PROJECT PRESENTATION',
                eventLogo: 'math/math.png',
                eventDescription: 'The project work constitutes a major component in most of the professional programmers. In this “PROJECT PRESENTATION” event, the project should be related to the field of Computer science / Information Technology. Participants of this event are expected to demonstrate some real life projects. These projects may be carried out in some industry, research and development laboratories, educational institutions or software companies by the participant. It is suggested that the project is to be chosen which should have some direct relevance in day-to-day activities. Participants have to show the actual working of their project.  ',
                eventRules: [
                    "Each team/member will be given 10-15 minutes to demonstrate their project work.",
                    "Maximum 3 students per team/group are allowed.",
                    "Any student from any college and from any branch is allowed to participate but the project should be oriented to computer technology or it may be using some sort of computer programming.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Meha Nileshkumar Bhatt ',
                        contact: '9427620105'
                    },
                    {
                        name: 'Subham ',
                        contact: '8295841803'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Rachana Modi ',
                        contact: '9825015094'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 32,
                subEventName: 'Code Connection',
                eventLogo: 'math/math.png',
                eventDescription: 'Code connection is the coding competition where the participants will need to code for the interesting real life problems.1. Code Connection is a two person team event, in which one can showtheir talent using C programming language. Code Connection looksfor the real star team that stands up to the ability of ACCURACY,SPEED &amp; LOGIC. Come take part2. in the Code Connection by showcasing coding skills in different rounds. You will get various challenges like: 3. Can you construct the program of given output as a team? 4. Can you construct the program of given output by shuffling the team member? For example, the first member has to code for the first half of the given time and for the remaining time the second member has to code.5. Can you reconstruct the code snippets to make a working program that produces the given output individually or by shuffling team members? 6. Can you construct the program of given problem definition by shuffling the team member?  ',
                eventRules: [
                    " A group of two will form the participating team for the event.",
                    "Participants will be given a blank A4 size paper for doing rough work.",
                    "No external electronic data may be accessed at any time. This includes phones, USB drives, the internet, etc.", "The contest will consist of two rounds having a variety of problems.",
                    "The permissible programming language is C only.",
                    "All solutions will be submitted electronically. Solutions will be tested by the judges and feedback will be sent accordingly.", "Any unauthorized behavior will result in automatic disqualification.",

                    "This includes but is not limited to: looking at other’s work/solutions, using disallowed external resources, or attempting to hack the contest environment.", "Judges&#39; decisions are final and will not be challenged in any case. ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Dhruvin Kachhadia ',
                        contact: '9974188758'
                    },
                    {
                        name: 'Triaa Shah ',
                        contact: '9099961103'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Ravi Raval ',
                        contact: ''
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 33,
                subEventName: 'WEBATHON',
                eventLogo: 'math/math.png',
                eventDescription: 'This event exposes your website design skills. Participant must have knowledge about CSS3, HTML5, JAVASCRIPT',
                eventRules: [
                    "Round 1 (Elimination round) : In Round One there will be one quiz which is based on HTML, CSS, JavaScript and Bootstrap. After quiz there 15 groups will be shortlist. (Optional)",
                    "Round 2(Preliminary Round) : In Round 2 Participants have to design web page as given demo web page in a time period.",
                    "Make template using HTML and CSS",
                    "Coding allowed only in NOTEPAD",
                    "Use of Paint would be allowed",
                    "Round 3 (Final Round) :",
                    "In Round 3 Shortlisted students have to Design Web page. Title will be given at that time. (Grading decision will be taken by Expert or Faculty Coordinators) ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Venish Sureliya',
                        contact: '9723215199'
                    },
                    {
                        name: 'Kanishka Khandelwal',
                        contact: '9104748482'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Chirag gami ',
                        contact: '9.01641367798243E+29'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 34,
                subEventName: 'CRYPT YOUR  MIND',
                eventLogo: 'math/math.png',
                eventDescription: 'CRYPT YOUR MIND” is a technical feast for techno and logical minds. In this event, students will start one offline game. Those who complete the game first will unlock the second round first. In round-2, Students need to solve Scrambled words. Round-3 is the elimination round.',
                eventRules: [
                    "Round 1 (head start): Basic game like google chrome offline game",
                    "Round 2: Cheats of a single phase will be given to the students and they have to arrange those words and make a sentence. Words will be jumbled.",
                    "Round 3: Buzzer round (Elimination round).",
                ],
                cordinatorsContact: [
                    {
                        name: 'Manthan Mehta ',
                        contact: '7048501016'
                    },
                    {
                        name: 'Vriddhi Shah ',
                        contact: '9873197886'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Yogesh Prajapati ',
                        contact: '7984011603'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '1500',
                    third: '1000'
                },
            },

        ]
    },
    {
        EventId: 4,
        EventName: 'Electabuzz',
        MajorEventLogo: 'electabuzz.jpg',
        subEvents: [
            {
                subEventId: 24,
                subEventName: 'Circuit assembly',
                eventLogo: 'math/math.png',
                eventDescription: 'Basic Electrical Circuit Making & Assembly ',
                eventRules: [
                    "Rules & Regulations:",
                    "A tool kit of Electronics Devices will provide to the team.",
                    "Participants will have to assemble the circuit before given time limit.",
                    "They will have to assemble right circuit on bread board or PCB.",
                    "Ranking to the them will be given on the basis of time bound assembly work and technical knowledge.",
                    "First three ranker will be winner.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Devariya Jignesh R.',
                        contact: '7434029456'
                    },
                    {
                        name: 'Maheshwari Harsh D. ',
                        contact: '7359091033'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Naman Bhatt',
                        contact: '9033173719'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },

            {
                subEventId: 26,
                subEventName: 'Think beyond',
                eventLogo: 'math/math.png',
                eventDescription: 'Updated soon ',
                eventRules: [
                    "Idea must reflect the innovativeness and its feasibility of the project.",
                    "Maximum three participants are allowed per team.",
                    "Each group have to prepare a chart or model or poster approximately A2 size.",
                    "The total time duration for explaining the idea and time will be given only 10 minutes (8 minutes for explanation + 2 minutes for question-answer)",
                    "Participants from same institutes and department can form a team.",
                    "All the participants will be awarded participation certificate.",
                    "Certificate of Excellence will be awarded to Winning teams. ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Praveen Rawat',
                        contact: '8829089151'
                    },
                    {
                        name: 'Snehal ganvit ',
                        contact: '8155814491'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Naman bhatt',
                        contact: '9033173719'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 27,
                subEventName: 'Projectron',
                eventLogo: 'math/math.png',
                eventDescription: 'Working model competition ',
                eventRules: [
                    "Maximum three participants are allowed per team",
                    "Model of project must be required",
                    "Model for PROJECTRON must be related to Electrical Engineering",
                    "The total time duration for explaining the model will be 10 minutes. (8 minutes for explanation + 2 minutes for question-answer)",
                    "Participants from same institutes and department can form a team.",
                    "All the participants will be awarded participation certificate. Certificate of Excellence will be awarded to Winning teams",
                    "Measuring instruments & required supply should be provided by us only, other instruments and model equipment's will not provide.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Ashvin Parmar',
                        contact: '8160443998'
                    },
                    {
                        name: 'Ayush Chauhan ',
                        contact: '8849321892'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Naman bhatt',
                        contact: '9033173719'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 30,
                subEventName: 'Mind Spark',
                eventLogo: 'math/math.png',
                eventDescription: 'This is the event where student will appear in a quiz, Group Discussion & Mock Interview  ',
                eventRules: [
                    "Eligibility for Junior Electrical Engineer level: Student from diploma final year and B.Tech/B.E. Second year.",
                    "Eligibility for Deputy Electrical Engineer level: Student from B.Tech/B.E. Third year and final year.",
                    "Eligibility for Senior Electrical Engineer level: Student from B.Tech/B.E. Final year and M.Tech/M.E.",
                    "Participants should come in a formal dress code and bring only photocopy of necessary documents with their identification proof.Elimination Round:", "A technical test of 20 Marks will be conducted.",
                    "Total time duration will be 15 Minutes.",
                    "Student who will be shortlisted can proceed in the next round.",
                    "Group Discussion:",
                    "From the short listed participants, groups for discussion will be formed on the basis of online test raking.",
                    "The individual performance during group discussion will be judged by penal of experts.",
                    "Student who will be shortlisted can proceed in the next round.",
                    "Interview:",
                    "A panel of experts will ask technical, managerial and general knowledge related questions and will give their Judgment. ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Suthar Krupesh ',
                        contact: '7359528930'
                    },
                    {
                        name: 'Patel Ruchitkumar',
                        contact: '9016591317'
                    },
                ],
                facultyContact: [
                    {
                        name: 'prof. N. J. bhatt',
                        contact: '9033173719'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
        ]
    },
    {
        EventId: 5,
        EventName: 'MariTech',
        MajorEventLogo: 'maritech.jpg',
        subEvents: [
            {
                subEventId: 1,
                subEventName: 'Boat Racing ',
                eventLogo: 'math/math.png',
                eventDescription: 'Complete your race without sinking your boat which may eliminate your boat.Round 1:-Place your RC boat in water track.Complete the path in minimum time.The obstacles will be minimum in the first round.First five participants will qualify for the round 2.Round 2:-The obstacles will much more in this round.The first three participants will be awarded.Complete your race in minimum time.',
                eventRules: [
                    "This is the open contest and anyone can take part .",
                    "The team should not have more than 3 students.",
                    "The maximum width of the RC boat is 20cm.",
                    "The maximum length of the RC boat is 45cm.",
                    "The maximum voltage of  the RC boat is 12V.",
                    "The RC boat made by students only, Readymade boat  should not allowed.",
                    "Engine power boats are strictly not permitted.",
                    "Participants are not allowed to make any changes in the boat after participating in first round.",
                    "Any damaged to the RC boat, owners will be responsible.",
                    "Only one team member will allow to control the remote of the boat, controller should not be change through the event.",
                    "The penalty of 15 Seconds will be added, if the team member will touch the boat after putting the RC boat in water.",
                    "The time will not stop until  the race will complete."

                ],
                cordinatorsContact: [
                    {
                        name: 'Nirjhar Sarkar ',
                        contact: '8583038477'
                    },
                    {
                        name: 'Nitant Kumar ',
                        contact: '9341837672'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel ',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },


            {
                subEventId: 2,
                subEventName: 'Ship Building ',
                eventLogo: 'math/math.png',
                eventDescription: 'Participants will be provided with all the materials required to build the ship.They will be guided by the co-ordinators.After the completion of the ship-building the ship would have to go through floating test.Every participant will be provided with certificate.Ship building will commence on 14th followed by floating test on 15th of oct 2022.All the ships will be judged without any partiality  by the judges.The decision of the judges would be final.There would be two rounds in this event.Every participating group would be charged a participation fees of 500/- rupees only.Participants has to bare an additional fee of 250/- rupees after the last date of registration i.e. 13/10/22.',
                eventRules: [
                    "The registration must be done prior to the commencement of the event.",
                    "If failed to register before the given time, then they would be charged with extra entry fees for late registration and no extra time will be provided prior to testing.",
                    "Only existing students of any Diploma or B.Tech. course may participate in this competition. ",
                    "The team must not have more than five students.",
                    "Using any extra material brought by the participants will not be allowed.",
                    "All the participants will be provided with same materials for fair competency.",
                    "The ship would be checked for flotation period.",
                    "The ship might be tested against artificially created waves."

                ],
                cordinatorsContact: [
                    {
                        name: 'Shivam Yadav  ',
                        contact: '7007166759'
                    },
                    {
                        name: 'Abhi Gajjar ',
                        contact: '6352777986'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel ',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },


            {
                subEventId: 3,
                subEventName: 'Paper Presentation ',
                eventLogo: 'math/math.png',
                eventDescription: 'Present a paper in any topic related to renewable & green energy.',
                eventRules: [
                    "Participant should have to submit the word file & ppt file of paper 1 day before the event.",
                    "Participant should present the PPT in front of jury, judge & coordinator."

                ],
                cordinatorsContact: [
                    {
                        name: 'Aryan Kurane   ',
                        contact: '8652359400'
                    },
                    {
                        name: 'Harsh Panchal ',
                        contact: '9321698683'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel ',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 4,
                subEventName: 'Poster Presentation',
                eventLogo: 'math/math.png',
                eventDescription: "Make a poster on A2 sheet.",
                eventRules: [
                    "Make it on the spot."

                ],
                cordinatorsContact: [
                    {
                        name: 'Himanshu',
                        contact: '8146045956'
                    },
                    {
                        name: 'Anupal Dev',
                        contact: '7577809495'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel ',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },


            {
                subEventId: 5,
                subEventName: 'Project Marino',
                eventLogo: 'math/math.png',
                eventDescription: "Make a working model & demonstrate it",
                eventRules: [
                    "Make it by hand, don't buy it from market."

                ],
                cordinatorsContact: [
                    {
                        name: 'Suryadev Singh ',
                        contact: '9724107889'
                    },
                    {
                        name: 'Ronak Rathore ',
                        contact: '7096080967'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel ',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 25,
                subEventName: 'Model Mania',
                eventLogo: 'math/math.png',
                eventDescription: 'Visible Examination - Judges will visual examine the model.Explanation - participants will be explain the working their respectively model Working test - participants have to show their model working ',
                eventRules: [
                    "The model should be working.",
                    "The model should be related to topic.",
                    "Maximum 4 participants are allowed.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Suryadev Singh',
                        contact: '9724107889'
                    },
                    {
                        name: 'Ronak Rathore ',
                        contact: '7096080967'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Hardik A Patel',
                        contact: '9106593769'
                    },
                ],
                pricePool: {
                    first: '5000',
                    second: '3000',
                    third: '1000'
                },
            },
        ]
    },
    {
        EventId: 6,
        EventName: 'Mech-Mechato',
        MajorEventLogo: 'mech_mechato.png',
        subEvents: [
            {
                "subEventId": 1,
                "subEventName": "CAD clash",
                "eventLogo": "sub/cad_clash.jpg",
                "eventDescription": "C.A.D (Computer-aided design) competition where participants need to fulfill the needs of completing the model in part, assembly, and mechanism.",
                "eventRules": [
                    "Each candidate will prepare a model using 'SolidWorks' or 'AutoCAD' software.",
                    "This event will be conducted in two rounds.",
                    "Events will be carried out by coordinates. If any misbehavior is found, the candidate will be disqualified.",
                    "Mobile/electronic devices are strictly prohibited.",
                    "The final decisions of the event are dependent on experts.",
                    "Instructions:",
                    "Round 1: The first round involves modeling of parts in two sessions. Students may use AutoCAD or SolidWorks software for the preparation of parts. The time taken for the preparation is noted for the selection of the best candidates for the second round.",
                    "Round 2: This is the final round of the competition, where participants have to go for Advanced modeling, Assembly, and mechanism. The best performer will be selected as the winner."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Patel Vrushik",
                        "contact": "9825397210"
                    },
                    {
                        "name": "Patel Viraj",
                        "contact": "9510531347"
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Dr. Harshadkumar Patel",
                        "contact": "9662052779"
                    }
                ],
                "pricePool": {
                    "first": "2500",
                    "second": "2000",
                    "third": "1500"
                },
            },
            {
                "subEventId": 2,
                "subEventName": "AUTOMOBILE WORKSHOP",
                "eventLogo": "sub/automobile_workshop.jpeg",
                "eventDescription": "Automobile Workshop is an event in which the participants will have hands-on training on technical maintenance activities of a vehicle like OBD Car Scanning, Wheel Balancing, Spark Plug testing, General Vehicle check-up, etc.",
                "eventRules": [
                    "Follow workshop safety rules."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Ajay Shibu",
                        "contact": "9574823113",
                    },
                    {
                        "name": "Ish Hingrajia",
                        "contact": "9924191084",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Prof. Jaimin Brahmbhatt",
                        "contact": "9737841171",
                    },
                    {
                        "name": "Prof. Ashish Sharma",
                        "contact": "9824804346",
                    }
                ],
                "pricePool": {
                    "first": "2500",
                    "second": "2000",
                    "third": "1500"
                },
            },
            {
                "subEventId": 3,
                "subEventName": "Treasure Hunt",
                "eventLogo": "math/1536.png",
                "eventDescription": "Gather your team of intrepid explorers and prepare to decode clues, solve puzzles, and navigate through a series of thrilling challenges scattered across the campus. From hidden corners to iconic landmarks, every clue brings you closer to uncovering the ultimate treasure.",
                "eventRules": [
                    "Maximum 3 members in each team are allowed.",
                    "Each team has to announce a leader at the time of registration.",
                    "There is a time limit of 4 hrs for all the competing teams to find the treasure.",
                    "The clues are to be found in a particular order."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Kirtan B Mangukiya",
                        "contact": "7817808400",
                    },
                    {
                        "name": "Ladani Shaan B",
                        "contact": "9512929140",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Ashish S Patel",
                        "contact": "9737396964",
                    },
                    {
                        "name": "Jigar V Patel",
                        "contact": "9099922072",
                    },
                ],
                "pricePool": {
                    "first": "2500",
                    "second": "1300",
                    "third": "No"
                },

            },
            {
                "subEventId": 39,
                "subEventName": "Tech Quiz",
                "eventLogo": "math/1536.png",
                "eventDescription": "A technical quiz is a type of quiz that focuses on testing knowledge and understanding of technical subjects, such as science, technology, and engineering fields. It may include questions related to concepts, theories, and problem-solving techniques.",
                "eventRules": [
                    "Based on Rounds",
                ],
                "cordinatorsContact": [
                    {
                        "name": "Gupta Daksh Rajiv",
                        "contact": "7265950896",
                    },
                    {
                        "name": "Dave Shubhendu Arunkumar",
                        "contact": "8153814282",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Puneet Bansal",
                        "contact": "96648584460",
                    }
                ],
                "pricePool": {
                    "first": "2000",
                    "second": "1500",
                    "third": "1000"
                },
            },
            {
                "subEventId": 41,
                "subEventName": "Mechatronics Engineering",
                "eventLogo": "math/1536.png",
                "eventDescription": "This event is for students proficient in Mechatronics Engineering.",
                "eventRules": [
                    "Only existing students of any Diploma or B.Tech. course may participate in this competition.",
                    "The team must not have more than four students.",
                    "Maximum wingspan of RC plane = 60 inches.",
                    "Minimum wingspan of RC plane = 42 inches.",
                    "Maximum weight of RC plane = 1500 grams.",
                    "Maximum Voltage of RC plane battery = 12.6 Volt (3s Battery)",
                    "Readymade RC Planes or any brand RC Planes are not allowed.",
                    "Only designed and fabricated RC Planes will be allowed.",
                    "Engine powered planes are strictly not permitted.",
                    "Participants are not allowed to make any changes in their RC plane after participating in the first round.",
                    "The battery terminals should be protected to avoid causing fire in the battery; failure will lead to direct disqualification.",
                    "Any damage to the RC plane will be the owner's responsibility.",
                    "Flight controller will not be allowed; only manual control plane will be allowed if found, then the coordinator will decide what to do."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Raj Pithva",
                        "contact": "8401413130"
                    },
                    {
                        "name": "Kartik Siddhpura",
                        "contact": "9725753557"
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Prof.Trushar R. Shah",
                        "contact": "7383928401"
                    }
                ],
                "pricePool": {
                    "first": "5000",
                    "second": "3000",
                    "third": "2000"
                }
            },
            {
                "subEventId": 42,
                "subEventName": "Project Presentation",
                "eventLogo": "sub/project_presentation.jpeg",
                "eventDescription": "Project Presentation for students in Mechanical Mechatronics.",
                "eventRules": [
                    "Poster/banner should be of 3ft by 3ft in size and it should contain the following topics:",
                    "1.Title",
                    "2.Problem formulation",
                    "3.Objective",
                    "4.Methodology",
                    "5.Outcome",
                    "6.Figure",
                    "7.Cost"
                ],
                "cordinatorsContact": [
                    {
                        "name": "Jay Chaudhary",
                        "contact": "9712482769",
                    },
                    {
                        "name": "Saket Salecha",
                        "contact": "9772754354",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Bhavesh.P.Patel",
                        "contact": "+91 82380 22850",
                    }
                ],
                "pricePool": {
                    "first": "5000",
                    "second": "3000",
                    "third": "2000"
                },
            },
            {
                "subEventId": 45,
                "subEventName": "PRINT FUSION 2K24",
                "eventLogo": "math/1536.png",
                "eventDescription": "Additive Manufacturing - Awareness & Industrial Application",
                "eventRules": [
                    "Workshop - Need to Attend by full Presence of Mind"
                ],
                "cordinatorsContact": [
                    {
                        "name": "Rathod Aditya",
                        "contact": "9726541896",
                    },
                    {
                        "name": "Rathod Dhruv",
                        "contact": "6355927687",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Dr. Achyut Trivedi",
                        "contact": "9265018153",
                    }
                ],
                "pricePool": {
                    "first": "1500",
                    "second": "1000",
                    "third": "500"
                },
            }
        ]
    },
    {
        EventId: 7,
        EventName: 'PetroX',
        MajorEventLogo: 'petrox.jpg',
        subEvents: [
            {
                subEventId: 21,
                subEventName: 'Chem-E-Car',
                eventLogo: 'math/math.png',
                eventDescription: 'This Event is based on Chemical Driven Innovative car ',
                eventRules: [
                    "Students should not use any kind of battery for running the car. The only energy source for the car to move is a chemical reaction.", "Students should not use hazardous chemicals.",
                    "The car should be an autonomous vehicle and should not be controlled remotely ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Man Maheshkumar Patel ',
                        contact: '8320266001'
                    },
                    {
                        name: 'Zeel Prakashbhai Patel ',
                        contact: '6353830778'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Anil Munde ',
                        contact: '8999901310'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 22,
                subEventName: 'Technical Poster Making Competition',
                eventLogo: 'math/math.png',
                eventDescription: 'The objective of the event to sensitize and aware everyone very critical issues related to the health, safety and environment and related latest developments   ',
                eventRules: [
                    "Time allotted: One hour",
                    "You will be given A2 size drawing sheet.",
                    "You will be required to bring you own pencil, eraser, pen, colors and drawing instruments.",
                    "You cannot paste any ready printout.",
                    "You will be required to present your poster to the judge.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Vaidehi Dineshbhai Prajapati ',
                        contact: '6359002715'
                    },
                    {
                        name: 'Sarthak Jigneshbhai Patel ',
                        contact: '9173650776'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. Vijay N Patel ',
                        contact: '9979487201'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 23,
                subEventName: 'WASTE-O-INDUSTRY MODEL',
                eventLogo: 'math/math.png',
                eventDescription: 'This event is based on to make an industrial model with use of waste materials ',
                eventRules: [
                    "The student will give the 1.5 Hr. for making their industrial model",
                    "Student can not use any digital device, virgin materials during model making   ",
                ],
                cordinatorsContact: [
                    {
                        name: 'Het Jagdishbhai Patel ',
                        contact: '6351951052'
                    },
                    {
                        name: 'Vishal Ramabhai Raval ',
                        contact: '9924595370'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Dr. Deoashish Panjiara ',
                        contact: '7070485221'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
        ]
    },
    {
        EventId: 8,
        EventName: 'Biotechnica',
        MajorEventLogo: 'biotechnica.png',
        subEvents: [
            {
                subEventId: 1,
                subEventName: 'Poster Presentation',
                eventLogo: 'math/math.png',
                eventDescription: "Join us for an immersive exploration into the cutting-edge advancements in biomedical and biotechnology research shaping the future of healthcare. Our poster presentation event offers a platform for students and scholars to showcase their latest findings, breakthroughs, and innovations in these dynamic fields.",
                eventRules: [
                    "1.	Each participant or group is permitted to display only one poster.",
                    "2.	Each group is limited to a maximum of three students. ",
                    "3.	All posters must feature original ideas and content. ",
                    "4.	Posters should directly relate to the designated theme of the event. ",
                    "5.	Text and graphics on posters must be clear, legible, and visible from a distance of at least 2 feet. ",
                    "6.	Posters should adhere to the A2 size standard, measuring 16.5 inches by 23.4 inches ",
                    "7.	Titles of posters should be concise, relevant, and accurately reflect the content presented. ",

                ],
                cordinatorsContact: [
                    {
                        name: 'Divyanshi Singh Gautam',
                        contact: '8128603979'
                    },
                    {
                        name: 'Archana Nair',
                        contact: '8980531907'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. T. V. Bhatt ',
                        contact: '9426999008'
                    },
                    {
                        name: 'Dr. Sneeha Veerakumar',
                        contact: '9426999008'
                    },
                ],
                pricePool: {
                    first: '3000',
                    second: '2000',
                    third: '1000'
                },
            },
            {
                subEventId: 2,
                subEventName: 'Jugaad Bio Art',
                eventLogo: 'math/math.png',
                eventDescription: "'Recycling things is like magic that turns ordinary things into extraordinary'. The event is based upon the creativity and innovation. The aim is to get maximum practical benefits from products and to generate the minimum amount of waste. The Jugaad Bio Art simply means to make or to create innovative and attractive things from the material we do not use anymore and relevant to the  biotechnica event.",
                eventRules: [
                    "The one group must not exceed more than 3 students. Students ideas should not get repeated again and again. Students will get maximum 2 hours for the preparation of their technical model."

                ],
                cordinatorsContact: [
                    {
                        name: 'Purva Vekariya ',
                        contact: '8758750150'
                    },
                    {
                        name: 'Krishika Chauhan',
                        contact: '8160838716'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Mansi Singh ',
                        contact: '9328829004'
                    },
                    {
                        name: 'Kalpana Adarkhi',
                        contact: '7283846110'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 28,
                subEventName: 'Quizophile',
                eventLogo: 'math/math.png',
                eventDescription: 'There will be a quiz competition between participants based on technical area, current affairs etc. It will be having 4 rounds of the quiz consist of different formats like rapid fire round , Buzzer round, audio &visual round and general around.  ',
                eventRules: [
                    "On the immediate completion of a question, each team gets 5 seconds to answer it. If a team is not able to answer a question, they can pass it for the next question. There is no team discussion in this round. 10 points is awarded for the correct answer.",
                    "Buzzer Round : 10 questions will be fired at all the teams one after another. The teams can discuss among themselves and then press the buzzer/bell to answer the question first. No discussion is allowed after pressing the buzzer. The question has to be answered in 20 seconds. If a question is not answered by the first team who pressed the bell. The team that pressed the bell next gets to answer. The team that presses the buzzer/bell first gets a chance to answer it. 10 points for the correct answer and minus 5 points for the wrong answer. If a team doesn’t answer or gives wrong answers after they press buzzer they also lose 5 points.",
                    "Audio-Visual round : Each Team will be shown a picture with a quotation. The team has to complete the quotation. The team has 15 seconds to complete the quotation. The teams will be made to listen to an audio clip. The team has to answer the questions based on the audio clip. Questions will not be passed to the next team.",
                ],
                cordinatorsContact: [
                    {
                        name: 'Patidar Vishvan Jayeshkumar ',
                        contact: '7226863570'
                    },
                    {
                        name: 'Desai Esha Sanjaykumar ',
                        contact: '7043313328'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Dr. Tejesh Reddy ',
                        contact: '9866979960'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 29,
                subEventName: 'Food Fiesta',
                eventLogo: 'math/math.png',
                eventDescription: 'Food fiesta is both food competition and show event. ',
                eventRules: [
                    ""
                ],
                cordinatorsContact: [
                    {
                        name: 'Paridhi Gandhi',
                        contact: '6351396609'
                    },
                    {
                        name: 'Dhairya Jani ',
                        contact: '6355399770'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Sonupriya Roy',
                        contact: '8511611551'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            {
                subEventId: 7,
                subEventName: 'innovative mania',
                eventLogo: 'math/math.png',
                eventDescription: "We welcome new innovative ideas And Provide Stage For the Students So they can Perform their Innovative Ideas & Represent Their Models.",
                "eventRules": [
                    "The project may be undertaken by individual student or in group not exceeding three members.",
                    "There will be 2 stage assessments of Projects for final selection. I. PHASE I, which is Pre-assessment stage, Based on the project abstract submitted by you. II. PHASE II which is final assessment stage, Based on Video demonstration of working of project send by you.",
                    "The participants will have to send their Project abstract on or before 10 march 2022, 4:00 PM to hkp01@ganpatuniversity.ac.in with the Title of your project.",
                    "There should be a title, name of team members, college name, email address and mobile number at the top of the abstract.",
                    "The abstract will be selected on the basis of; I. Addresses the critical challenges of the current health care scenario. II. Innovation - either replacing the current technology or your own idea. III. Economic and feasible aspects.",
                    "After the selection of the abstract, the participants must make a video demonstrating the working of their project and send it in CD/DVD on or before 10 march 2024, 4:00 PM to the event coordinator.",
                    "Shortlisted projects for participation in the competition will be informed by e-mail & published in our website from time to time.",
                    "You must Give your Requirements list in Advanced."
                ],
                cordinatorsContact: [
                    {
                        name: 'Shubh Patel & Khushi sharma',
                        contact: '8200645021'
                    },
                    {
                        name: 'Daivik Patel & Mahima patel',
                        contact: '9173823760'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Heena Patel',
                        contact: '9737177328'
                    },
                ],
                pricePool: {
                    first: '3000',
                    second: '2500',
                    third: '2000'
                },
            },
            {
                subEventId: 9,
                subEventName: 'Food Fiesta',
                eventLogo: 'math/math.png',
                eventDescription: "It is a both food competition and show event.",
                "eventRules": [
                    "A team should contain 2 to 4 member.",
                    "Token fees of Rs 100 has to be paid by interested team.",
                    "The project may be undertaken by individual student or in group not exceeding three members.",
                    "There will be 2 stage assessments of Projects for final selection. I. PHASE I, which is Pre-assessment stage, Based on the project abstract submitted by you. II. PHASE II which is final assessment stage, Based on Video demonstration of working of project send by you.",
                    "The participants will have to send their Project abstract on or before 10 march 2022, 4:00 PM to hkp01@ganpatuniversity.ac.in with the Title of your project.",
                    "There should be a title, name of team members, college name, email address and mobile number at the top of the abstract.",
                    "The abstract will be selected on the basis of; I. Addresses the critical challenges of the current health care scenario. II. Innovation - either replacing the current technology or your own idea. III. Economic and feasible aspects.",
                    "After the selection of the abstract, the participants must make a video demonstrating the working of their project and send it in CD/DVD on or before 10 march 2024, 4:00 PM to the event coordinator.",
                    "Shortlisted projects for participation in the competition will be informed by e-mail & published in our website from time to time.",
                    "You must Give your Requirements list in Advanced."
                ],
                cordinatorsContact: [
                    {
                        name: 'Paridhi Gandhi',
                        contact: '6351396609'
                    },
                    {
                        name: 'Dhariya Jani',
                        contact: '6355399770'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Nidhi G. Patel',
                        contact: '8128684301'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
            
            
            {
                subEventId: 10,
                subEventName: 'TECHNO WORKSHOP',
                eventLogo: 'math/math.png',
                eventDescription: "Dive into the world of interconnected devices at our technical workshop on the Internet of Things (IoT). Learn how to harness the power of IoT to create innovative solutions, connect devices, and gather meaningful data for improved decision-making. Gain hands-on experience with IoT hardware and software, and explore the endless possibilities of this rapidly evolving field.  ",
                "eventRules": [
                    "Registration will be based on first come first serve basis.",
                    "Certificate will be given to the each participant.",
                    "A team should contain 2 to 4 member.",
                    "Token fees of Rs 100 has to be paid by interested team.",
                    "The project may be undertaken by individual student or in group not exceeding three members.",
                    "There will be 2 stage assessments of Projects for final selection. I. PHASE I, which is Pre-assessment stage, Based on the project abstract submitted by you. II. PHASE II which is final assessment stage, Based on Video demonstration of working of project send by you.",
                    "The participants will have to send their Project abstract on or before 10 march 2022, 4:00 PM to hkp01@ganpatuniversity.ac.in with the Title of your project.",
                    "There should be a title, name of team members, college name, email address and mobile number at the top of the abstract.",
                    "The abstract will be selected on the basis of; I. Addresses the critical challenges of the current health care scenario. II. Innovation - either replacing the current technology or your own idea. III. Economic and feasible aspects.",
                    "After the selection of the abstract, the participants must make a video demonstrating the working of their project and send it in CD/DVD on or before 10 march 2024, 4:00 PM to the event coordinator.",
                    "Shortlisted projects for participation in the competition will be informed by e-mail & published in our website from time to time.",
                    "You must Give your Requirements list in Advanced."
                ],
                cordinatorsContact: [
                    {
                        name: 'Tadha Qamaruz Zaman ',
                        contact: '7359392029'
                    },
                    {
                        name: 'Ayushi Parmar',
                        contact: '9429425352'
                    },
                ],
                facultyContact: [
                    {
                        name: 'Prof. T. B. Prajapati & Prof. D. M. Jarecha',
                        contact: '9537615073'
                    },
                ],
                pricePool: {
                    first: '2500',
                    second: '2000',
                    third: '1500'
                },
            },
        ]
    },
    {
        EventId: 9,
        EventName: 'DipTechCon',
        MajorEventLogo: 'DipTechCon.jpg',
        subEvents: [
            {
                "subEventId": 1,
                "subEventName": "Webathon",
                "eventLogo": "math/1536.png",
                "eventDescription": "The Webathon event presents an exciting opportunity for coders of all skill levels to showcase their talents in a basic website competition. Participants will utilize their HTML, CSS, and JavaScript knowledge to design and develop functional and creative websites within a specified time frame.",
                "eventRules": [
                    "Participants enrolled in Diploma Studies and interested in showcasing their skills in website development are invited to compete in this event.",
                    "Each team participating in the website competition may have up to four members.",
                    "The website should be a single page and only developed using HTML, CSS, and Javascript.",
                    "Prizes will be awarded to the competition winners, while all participants will receive certificates of participation.",
                    "Please note that additional rules and guidelines may be provided at the start of the competition.",
                    "This event is only for Diploma Students, so please carry a College ID card with you on the day of the event for verification.",
                    "Students other than Diploma Studies are not allowed to enroll or participate in the event, but they can participate in other events of Convergence 2024.",
                    "Participants/teams found violating the rules may get disqualified from the event."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Vivek Isha",
                        "contact": "9182740234",
                    },
                    {
                        "name": "Isha Patel",
                        "contact": "9586822850",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Prof. Ankit Raval",
                        "contact": "9909467847",
                    }
                ],
                "pricePool": {
                    "first": "2500",
                    "second": "1500",
                    "third": "1000"
                },
            },
            {
                "subEventId": 40,
                "subEventName": "CAD Cam",
                "eventLogo": "math/1536.png",
                "eventDescription": "This event is for those students who are proficient in software like AutoCAD and want to pursue their job / business in the design / drafting department of industry.",
                "eventRules": [
                    "Students participating in this event must be proficient in AutoCAD (2D & 3D).",
                    "This event is only for Diploma Students, so please carry a College ID card with you on the day of the event for verification.",
                    "There are two rounds in this event. The first round is an elimination round in which 2D drawing/ objects are given to students for drafting. Those students who have succeeded in this round will be promoted to the final round of 3D Drawing.",
                    "An isometric component is given to students for the last round.",
                    "Maximum 45 Minutes are given to students in both rounds.",
                    "Please note that additional rules and guidelines may be provided at the start of the competition.",
                    "Students other than Diploma Studies are not allowed to enroll or participate in the event, but they can participate in other events of Convergence 2024.",
                    "Participants/teams found violating the rules may get disqualified from the event."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Tadvi Vishramkumar Ravidas",
                        "contact": "8780344432",
                    },
                    {
                        "name": "Patel Het Shitalkumar",
                        "contact": "8849458720",
                    }
                ],
                "facultyContact": [{
                    "name": "Shaileshkumar Jayantilal Patel",
                    "contact": "9426394241",
                },],
                "pricePool": {
                    "first": "2500",
                    "second": "1500",
                    "third": "1000"
                },
            },
            {
                "subEventId": 43,
                "subEventName": "TechnoHunt",
                "eventLogo": "math/1536.png",
                "eventDescription": "Embark on a journey of discovery and problem-solving with our Technical Treasure Hunt! This thrilling event combines elements of technology, puzzles, and teamwork as participants race against the clock to uncover hidden clues, solve technical challenges, and unlock the ultimate prize.",
                "eventRules": [
                    "1. Participants eager to put their puzzle-solving and technical skills to the test are invited to join the Technical Treasure Hunt.",
                    "2. Teams of up to 4 members will navigate a series of challenges, puzzles, and tasks in search of hidden clues and treasures.",
                    "3. Prizes will be awarded to the top-performing teams who complete all challenges and reach the final destination.",
                    "4. Please stay tuned for additional rules and guidelines, which will be provided at the start of the Technical Treasure Hunt.",
                    "5. This event is only for Diploma Students, so please carry a College ID card with you on the day of the event for verification.",
                    "6. Students other than Diploma Studies are not allowed to enroll or participate in the event, but they can participate in other events of Convergence 2024.",
                    "7. Participants/teams found violating the rules may get disqualified from the event."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Vrunda Ghadvi",
                        "contact": "7984096962",
                    },
                    {
                        "name": "Siddarth Loni",
                        "contact": "9272252368",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Prof. Vidhi Chaudhary",
                        "contact": "7600561528",
                    }
                ],
                "pricePool": {
                    "first": "2000",
                    "second": "0",
                    "third": "0"
                },
            },
            {
                "subEventId": 44,
                "subEventName": "Drone Workshop and Competition",
                "eventLogo": "math/1536.png",
                "eventDescription": "The event is divided into two parts, the first part is for students who want to learn about drones and their applications while the second part is a drone competition in which drone pilots from across the state are invited to participate.",
                "eventRules": [
                    "1. Students who just want to learn about drones and its applications can also participate in this event; attending the drone workshop is not necessary to participate in the drone competition. There is a prize for the students who win the drone competition while a certificate will be provided to the students who participate in the workshop.",
                    "2. A Team with a maximum of 4 participants is allowed in the drone competition.",
                    "3. Readymade and Assembled drones are allowed.",
                    "4. Please note that additional rules and guidelines may be provided at the start of the competition.",
                    "5. This event is only for Diploma Students, so please carry a College ID card with you on the day of the event for verification.",
                    "6. Students other than Diploma Studies are not allowed to enroll or participate in the event, but they can participate in other events of Convergence 2024.",
                    "7. Participants/teams found violating the rules may get disqualified from the event."
                ],
                "cordinatorsContact": [
                    {
                        "name": "Kishan Changela",
                        "contact": "7862092677",
                    },
                    {
                        "name": "Neel Bhalodiya",
                        "contact": "6354963482",
                    }
                ],
                "facultyContact": [
                    {
                        "name": "Prof. Ishan Thakkar",
                        "contact": "9714520498",
                    }
                ],
                "pricePool": {
                    "first": "2500",
                    "second": "1500",
                    "third": "1000"
                },
            },

        ]
    },
]

export default Events;